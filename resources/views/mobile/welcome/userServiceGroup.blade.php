...
<section class="padding-x">
    <div class="row">
        ...
        <div class="col-6 col-sm-6 col-md-4 py-2">
            <a href="{{url('/', $user->userSecond->username)}}" class="product-sm mb-3">
                <div class="img-wrap"> <img src="{{ asset($user->userSecond->latestCheckedPP()) }}"
                        alt="{{ $user->userSecond->username }}">
                </div>
                <div class="text-wrap">
                    <p class="title text-truncate w3-small">
                        {{Str::limit($user->userSecond->name, 16,'..')}}<i
                            class="fa fa-circle {{ $user->userSecond->isOnline() ? 'w3-text-green' : 'w3-text-light-gray' }} w3-small"></i>
                    </p>
                    <p class="title text-truncate w3-small"> {{Str::limit($user->userSecond->religion, 20)}},</p>
                    <p class="title text-truncate w3-small">{{Str::limit($user->userSecond->gender, 20)}},
                        {{Str::limit($user->userSecond->height, 20)}}</p>
                    <p class="title text-truncate w3-small">{{Str::limit($user->userSecond->marital_status, 20)}}</p>
