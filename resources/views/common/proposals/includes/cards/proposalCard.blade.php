      ...
      <div class="col-sm-12">
        <div class="box box-widget w3-border w3-border-purple  no-margin">
          ...
          <div class="box-header with-border text-center">
            <h3 class="box-title">
              Proposal from <span class="w3-tag w3-purple w3-round w3-large"> <a target="_blank" class="w3-text-white" href="{{ url('/',$proposal->user->username) }}">{{ $proposal->user->name }} ({{ $proposal->user->username }})</a> </span> to <span class="w3-tag w3-purple w3-round w3-large"> <a target="_blank" class="w3-text-white" href="{{ url('/',$proposal->userSecond->username) }}"> {{ $proposal->userSecond->name }} ({{ $proposal->userSecond->username }}) </a> </span>
            </h3>
          </div>
          <div class="box-body">
...
<div class="panel-body">  
                 ...
                 {{ $proposal->user->proposalToMe()->whereAccepted(false)->count() }} <br>
                 <b>Proposal to me: </b>
                 {{ $proposal->user->proposalToMe()->whereAccepted(true)->count() }} <br>
                 <b>Proposal sent by me: </b>
                 {{ $proposal->user->proposal()->count() }} <br>
                 <b>Current Package: </b>
...
<div class="panel-body">  
                 ...
                 <b>Proposal Pending to me: </b>
                 {{ $proposal->userSecond->proposalToMe()->whereAccepted(false)->count() }} <br>
                 <b>Proposal to me: </b>
                 {{ $proposal->userSecond->proposalToMe()->whereAccepted(true)->count() }} <br>
                 <b>Proposal sent by me: </b>
                 {{ $proposal->userSecond->proposal()->count() }} <br>
                 <b>Current Package: </b>
                    ...
                    <div class="col-sm-6  w3-border {{ $proposal->checked ? 'w3-border-green w3-green' : 'w3-border-red' }} w3-round">
                      ...
                      <div class="checkbox" style="margin: 5px 0px;">
                        <label>
                          ...
                          data-url="{{ route('common3.proposalCheckedByAdmin', $proposal) }}"
                          type="checkbox"
