...
<form class="form-send-proposal" action="{{ route('user.sendProposalPost', $user) }}" method="post">
 ...
 <div class="form-group {{ $errors->has('comment') ? ' has-danger' : '' }}">
  ...
  <textarea class="form-control w3-border w3-padding {{ $errors->has('comment') ? ' w3-border-red' : '' }}" name="comment" rows="3" placeholder="Write something to {{ $user->username }}" id="comment">{{ old('comment') ?: 'Dear '. $user->name . ', I would like to send you my proposal. Please, accept it as if it helps us to start our conversation in a better way...' }}</textarea>
